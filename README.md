...
做题页从上到下依次设置：**词库 → 单元筛选 → 闯关模式 → 做题模式 → 题目类型 → 每次学习量 →（可选）记忆模式 / 自动朗读**，然后点「▶ 开始做题」。
...
- **答题进度与定位**：顶部进度条中绿色表示已完成，灰色表示未完成，青绿色高亮表示当前题；切换上一题、下一题或输入题号跳转时会自动滚动跟随当前题，尽量保持在进度条中间，首尾题则停留在对应边界。非全屏翻题时，题卡会定位到吸顶导航栏下方，避免手机端导航遮住题面首行或全屏按钮
