...
| 📅 艾宾浩斯记忆调度 | 大纲词汇按书末「4周25天打卡表」（+1/+3/+6/+12 天间隔）；核心词汇按书中 8 格周期（5分/30分/12时/1天/2天/4天/7天），到期自动横幅提醒一键清债。完成判定以当天实际作答为准：当天计划的每个 Unit 只要全部单词都在今天答过一遍——无论通过「开始复习」队列、记忆模式、普通「开始做题」还是智能练习——计划栏都会显示“当天目标已完成”；复习队列中断后重建时只推送当天还没答过的词，已练过的题不会整段重做 |
...
**记忆规律：**
- 大纲词汇 → 书末「4周25天打卡表」：按 **+1天/+3天/+6天/+12天** 间隔复习（如第9天学 Unit 9 并复习 Unit 3、6、8；Unit 14 的末次复习按 +12 应在第26天，超出25天表并入第25天）
