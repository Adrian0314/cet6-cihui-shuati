...
| 📅 艾宾浩斯记忆调度 | 大纲词汇采用「4周25天·词群适配计划」（仅 Unit 1-10）（+1/+3/+6/+12 天间隔）；核心词汇按书中 8 格周期（5分/30分/12时/1天/2天/4天/7天），到期自动横幅提醒一键清债。完成判定以当天实际作答为准：当天计划的每个 Unit 只要全部单词都在今天答过一遍——无论通过「开始复习」队列、记忆模式、普通「开始做题」还是智能练习——计划栏都会显示“当天目标已完成”；复习队列中断后重建时只推送当天还没答过的词，已练过的题不会整段重做 |
...
- **中途退出**：做题进行中会即时把当前题位、已答数量和已选答案写入本地存储；手机切到后台、刷新页面或关闭网页时，页面生命周期事件还会再次保存，重新打开后点击顶部「继续做题」即可接着完成同一队列。手机从其他应用返回后，恢复保护会忽略浏览器补发的旧「不会，跳过」事件：首次真实点击选项可正常作答，只有实际按下跳过按钮才会执行跳过；跳过点击还必须落在真实起手之后 1.5 秒内，且起手必须发生在切回前台之后，补发的旧触摸（含完整序列重放）会被整事件取消，切后台很久再回来时点击选项不会被误判为跳过，跳过按钮也不会无故闪动。4 周 25 天计划会同时保存当日进度，复习提醒也会显示实际的已完成题数和待完成题数；当天新 Unit 的全部单词学完后会记录为已完成，不会再生成同一 Unit 的整组复习；复习队列中断后重新开始时只推送当天尚未答过的词，已练过的题不会重复出现；进入新的一天后，旧日未完成队列会按新的每日计划重置（若前一天的任务实际已全部答完，会按作答记录补判并正常进入下一天），已答完但来不及清除的旧队列也会在重进时自动丢弃。
...
**记忆规律：**
- 大纲词汇 → 本站「4周25天·词群适配计划」（仅 Unit 1-10）：按 **+1天/+3天/+6天/+12天** 间隔复习（如第9天学 Unit 9 并复习 Unit 3、6、8；第22天完成最后一次计划复习，第23-25天自主查漏补缺）
- 核心词汇 → 书中每词 8 格周期：**5分/30分/12时/1天/2天/4天/7天**

**到期复习：**
- 打开网页时若有到期单词，做题页顶部显示绿色**复习横幅**，点「开始复习」四选一快刷一键清债，跳过视同答错
- 5分/30分 这类短周期在学习会话内实时提醒
- 4 周 25 天计划的完成判定以**当天实际作答**为准：当天计划的每个 Unit 全部单词都在今天答过一遍（不限做题入口），计划栏即显示「当天目标已完成」；跨天仍未完成时，计划重置为当天的完整目标，不沿用前一天的部分进度
