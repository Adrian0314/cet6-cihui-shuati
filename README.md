...
- **中途退出**：做题进行中会即时把当前题位、已答数量和已选答案写入本地存储；手机切到后台、刷新页面或关闭网页时，页面生命周期事件还会再次保存，重新打开后点击顶部「继续做题」即可接着完成同一队列。4 周 25 天计划会同时保存当日进度，复习提醒也会显示实际的已完成题数和待完成题数；进入新的一天后，旧日未完成队列会按新的每日计划重置
