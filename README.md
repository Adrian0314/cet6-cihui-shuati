...
- **中途退出**：做题进行中会即时把当前题位、已答数量和已选答案写入本地存储；手机切到后台、刷新页面或关闭网页时，页面生命周期事件还会再次保存，重新打开后点击顶部「继续做题」即可接着完成同一队列。手机从其他应用返回后，恢复保护会忽略浏览器补发的旧「不会，跳过」事件：首次真实点击选项可正常作答，只有实际按下跳过按钮才会执行跳过；跳过点击还必须落在真实起手之后 1.5 秒内，切后台很久再回来时点击选项不会被误判为跳过。4 周 25 天计划会同时保存当日进度，复习提醒也会显示实际的已完成题数和待完成题数；当天新 Unit 的全部单词学完后会记录为已完成，不会再生成同一 Unit 的整组复习；复习队列中断后重新开始时只推送当天尚未答过的词，已练过的题不会重复出现；进入新的一天后，旧日未完成队列会按新的每日计划重置（若前一天的任务实际已全部答完，会按作答记录补判并正常进入下一天），已答完但来不及清除的旧队列也会在重进时自动丢弃。
